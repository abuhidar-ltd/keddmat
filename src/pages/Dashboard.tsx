...
const Dashboard = () => {
  ...
  const canReceiveEmergency = merchantCategory
    ? isEmergencyCategory(merchantCategory)
    : false;
  ...
  useEffect(() => {
    if (user) {
      supabase
        .from("profiles")
        .select("page_slug, category, phone")
        .eq("user_id", user.id)
        .maybeSingle()
        .then(async ({ data }) => {
          ...
          if (data?.phone) {
            ...
            if (
              ADMIN_PHONES.some(
                (p) => cleaned.includes(p) || p.includes(cleaned),
              )
            ) {
    ...
    <div className="min-h-screen bg-[#EFF3F8]">
      ...
      <header className="bg-white border-b border-[#E5E7EB] shadow-sm">
        <div className="container flex items-center justify-between h-16 px-4">
          <h1 className="font-extrabold text-lg text-[#175A92]">
            {t("dashboard.title")}
          </h1>
          <div className="flex items-center gap-2">
            {pageSlug && (
              <Button
                ...
                size="sm"
                asChild
                className="border-[#165B91] text-[#165B91] hover:bg-[#165B91]/5"
              >
                <Link to={`/p/${pageSlug}`}>
      ...
      <main className="container py-6 px-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList
            className={`grid h-auto min-h-[52px] w-full max-w-2xl mx-auto mb-6 bg-white border border-[#E5E7EB] rounded-2xl p-1 shadow-sm ${tabCount === 6 ? "grid-cols-6" : tabCount === 5 ? "grid-cols-5" : "grid-cols-4"}`}
          >
            {canReceiveEmergency && (
              <TabsTrigger
                value="emergency"
                className="flex flex-col items-center gap-0.5 py-2 text-[10px] sm:flex-row sm:gap-2 sm:text-sm rounded-xl data-[state=active]:bg-[#165B91] data-[state=active]:text-white data-[state=active]:shadow-md"
              >
            ...
            >
              ...
              <span>{language === "ar" ? "اضافه الصور" : "Add Photos"}</span>
            </TabsTrigger>
            <TabsTrigger
              value="ratings"
              className="flex flex-col items-center gap-0.5 py-2 text-[10px] sm:flex-row sm:gap-2 sm:text-sm rounded-xl data-[state=active]:bg-[#175A92] data-[state=active]:text-white data-[state=active]:shadow-md"
            >
              <Star className="h-4 w-4" />
              <span>{language === "ar" ? "التقيمات" : "Ratings"}</span>
            </TabsTrigger>
            <TabsTrigger
              value="payments"
              className="flex flex-col items-center gap-0.5 py-2 text-[10px] sm:flex-row sm:gap-2 sm:text-sm rounded-xl data-[state=active]:bg-[#165B91] data-[state=active]:text-white data-[state=active]:shadow-md"
            ...
            >
              ...
              <span>{language === "ar" ? "صفحه المتجر" : "Store Page"}</span>
            </TabsTrigger>
            {isAdminPhone && (
              <TabsTrigger
                value="admin"
            ...
            <Card className="rounded-2xl border-[#E5E7EB] shadow-sm">
              <CardHeader className="border-b border-[#E5E7EB] bg-[#165B91]/5 rounded-t-2xl">
                <CardTitle className="flex items-center justify-between text-[#1A1A2E]">
                  <span className="text-base text-[#175A92]">
                    {t("dashboard.ratingsCount")} ({ratings.length})
                  </span>
                  {ratings.length > 0 && (
                    <div className="flex items-center gap-2">
