...
import { Users, CreditCard, Store, CheckCircle, XCircle, Eye, Package, Trash2, Star, MessageCircle, Phone, MapPin, Calendar, Wallet, AlertTriangle } from 'lucide-react';
...
interface Merchant { id: string; user_id: string; display_name: string | null; store_name: string | null; phone: string; page_enabled: boolean; page_slug: string | null; created_at: string; whatsapp_clicks?: number; call_clicks?: number; emergency_clicks?: number; emergency_whatsapp_clicks?: number; emergency_phone_clicks?: number; category?: string; address?: string; }
...
const AdminPanel = () => {
  ...
  const [kpis, setKpis] = useState<{ customers: number; craftsmen: number; whatsappClicks: number; callClicks: number; emergencyClicks: number }>({ customers: 0, craftsmen: 0, whatsappClicks: 0, callClicks: 0, emergencyClicks: 0 });
  const [emergencyDaily, setEmergencyDaily] = useState<Array<{ date: string; whatsapp: number; phone: number; total: number }>>([]);
  ...
  const adminAction = async (type: string, data: any) => {
    try {
      ...
      if (!res.ok) {
        ...
        try { const errorBody = await res.json(); if (errorBody?.error) errorMessage = errorBody.error; } catch (_) {}
  ...
  if (loadingData) return <div className="flex items-center justify-center py-12"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div></div>;
  ...
  return (
    <div dir={dir}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">ضغطات الطوارئ</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-red-500" /><span className="text-2xl font-bold">{kpis.emergencyClicks}</span></div></CardContent></Card>
      </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            {language === 'ar' ? 'نقرات الطوارئ اليومية (آخر 30 يوم)' : 'Daily Emergency Clicks (Last 30 days)'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <ChartContainer
            config={{
              whatsapp: { label: language === 'ar' ? 'واتساب' : 'WhatsApp', color: 'hsl(142 70% 45%)' },
              phone: { label: language === 'ar' ? 'هاتف' : 'Phone', color: 'hsl(217 91% 60%)' },
            }}
            className="h-[260px] w-full"
          >
            <BarChart data={emergencyDaily} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <Legend />
              <Bar dataKey="whatsapp" stackId="a" fill="var(--color-whatsapp)" radius={[0, 0, 0, 0]} />
              <Bar dataKey="phone" stackId="a" fill="var(--color-phone)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ChartContainer>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="merchants" className="w-full">
        <TabsList className="grid w-full max-w-2xl grid-cols-4 mb-6">
          <TabsTrigger value="merchants" className="gap-1"><Store className="h-4 w-4" /><span className="hidden sm:inline">{t('admin.providers')}</span></TabsTrigger>
          <TabsTrigger value="products" className="gap-1"><Package className="h-4 w-4" /><span className="hidden sm:inline">{t('admin.services')}</span></TabsTrigger>
          <TabsTrigger value="ratings" className="gap-1"><Star className="h-4 w-4" /><span className="hidden sm:inline">{t('admin.ratingsTab')}</span></TabsTrigger>
          <TabsTrigger value="payments" className="gap-1"><CreditCard className="h-4 w-4" /><span className="hidden sm:inline">{t('admin.paymentsTab')}</span></TabsTrigger>
        </TabsList>
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                        <div className="flex-1 space-y-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {m.page_enabled ? <Badge className="bg-emerald-500 text-white text-xs">{t('admin.active')}</Badge> : <Badge variant="secondary" className="text-xs">{t('admin.inactive')}</Badge>}
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                            ...
                            <span className="flex items-center gap-1"><AlertTriangle className="h-3.5 w-3.5 text-red-500" />{m.emergency_clicks || 0} {language === 'ar' ? 'نقرة طوارئ' : 'emergency'}</span>
                            {m.page_slug && <a href={`/p/${m.page_slug}`} target="_blank" className="text-primary hover:underline text-xs">/{m.page_slug}</a>}
                          </div>
                        </div>
                        <div className="flex gap-2 shrink-0">
                          <Button size="sm" variant={m.page_enabled ? "destructive" : "default"} onClick={() => handleToggleStore(m)} className="gap-1">
